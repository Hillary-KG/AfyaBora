...
class Login{
    ...
    if(isset($_POST['login'])){
        ...
        $user = pg_escape_string($conn,trim($_POST['username']));
        $userpsw = pg_escape_string($conn,trim($_POST['password']);
        ...
        if(!empty($user) && !empty($userpsw)){
            ...
            $sql = "SELECT * FROM `dashboard_clinicianLogin` WHERE user_name = '$user'";
